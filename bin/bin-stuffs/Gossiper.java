...
public class Gossiper implements IFailureDetectionEventListener, IEndPointStateChangePublisher
{
  ...
  public static void sleepWhileExperimentRunning() {
    ...
    while (sleep) {
      ...
      if (tmp.exists()) {
	Util.debug("- Experiment running, gossiper sleeping ");
	Util.sleep(500);
	continue;
      }
      Util.debug("- Experiment not running, gossiper continue ");
      break;
    }
  }
  
  
  public static void sleepIfConnected() {
    File tmp = new File("/tmp/fi/nodesConnectedFlag");
    if (tmp.exists()) {
      Util.debug("- Nodes connected, gossiper sleeping ");
      Util.sleep(10000);
    }
   }
  ...
  private class GossipTimerTask extends TimerTask  {
    ...
    public void run() {
      ...
      try  {
        ...
        // haryadi: don't broadcast (CAn't do this .. insert will fail somehow)
	sleepIfConnected();
          ...
          if ( gDigests.size() > 0 ) {
	       ...
	       check if he is back up */
            ...
            /* Gossip to a seed if we did not do so above, or we have seen less nodes
               ...
               The most straightforward check would be to check that all the 
	       seeds have been
               verified either as live or unreachable.  To avoid that computation 
	       each round,
               we reason that:

               either all the live nodes are seeds, in which case non-seeds 
	       that come online
               will introduce themselves to a member of the ring by definition,

               or there is at least one non-seed node in the list, in which case 
	       eventually
  ...
  // public final static int intervalInMillis_ = 10000;
  ...
  public void convict(InetAddress endpoint)
  {
	...
	Util.debug("    - Gossiper.convict " + endpoint + " is now dead.");
  ...
  public void removeEndPoint(InetAddress endpoint)
  {
    ...
    Util.debug("    - Gossiper.removeEndPoint " + endpoint);
  ...
  boolean sendGossip(Message message, Set<InetAddress> epSet)
  {
    ...
    // haryadi
    sleepWhileExperimentRunning();
    sleepIfConnected();
    //Util.debug("- Gossiper.sg (0) sendoneway");
    //logger_.info("- Gossiper.sendGossip (0) sendoneway " + to);
  ...
  // haryadi: this tells the endpoint state map
  ...
  void markAlive(InetAddress addr, EndPointState localState)
  {
	...
	Util.debug("    - Gossiper.markAlive addr=" + addr + " live=" + true);
  ...
  private void handleNewJoin(InetAddress ep, EndPointState epState)
  {
    ...
    // haryadi .. also print to std out see which one is faster
    System.out.println("Node " + ep + " is now part of the cluster");

    logger_.info("Node " + ep + " is now part of the cluster");

    
    // *************************************************
    // haryadi, IMPORTANT HACK ... let's just set it to true, so
   ...
   * (either a previously
   * unknown node or a generation change for a known node). 
   * If the node is new, it will be
   * initially marked as dead. It will be marked alive as soon as another piece 
   * of gossip
   * arrives. On the other hand if the node is already known (generation change), 
   * we will immediately mark it alive.
   *
   * @param ep endpoint
  ...
  private void handleMajorStateChange(InetAddress ep, EndPointState epState, boolean isKnownNode)
  {
    ...
    Util.debug("    - Gossiper.handle map.put ep=" + ep + " live="  + isKnownNode);
  ...
  synchronized void isAlive(InetAddress addr, EndPointState epState, boolean value)
  {
    ...
    // haryadi
    Util.debug("    - Gossiper.isAlive addr=" + addr + " live=" + value);
  ...
  */
  ...
  public void start(InetAddress localEndPoint, int generationNbr)
  {
	...
	Util.debug("    - Gossiper.handle map.put ep=" + 
		   localEndPoint_ + " live="  + localState.isAlive());
  ...
  public static class GossipDigestSynVerbHandler implements IVerbHandler
  {
    ...
    public void doVerb(Message message)
    {
	  ...
	  sleepIfConnected();
          ...
          //Util.debug("- GossiperDS.dv (0) sendoneway");
	  //logger_.info("- GossiperDS.doVerb (0) sendoneway");
  ...
  public static class GossipDigestAckVerbHandler implements IVerbHandler
  {
    ...
    public void doVerb(Message message)
    {
	  ...
	  sleepIfConnected();
          ...
          //Util.debug("- Gossiper.dv (0) sendoneway");
	  //logger_.info("- Gossiper.dv (0) sendoneway");
