...
public class Gossiper implements IFailureDetectionEventListener, IEndPointStateChangePublisher
{
  ...
  static StorageService ss = StorageService.instance;
  static String tokenPath = "/tmp/fi/tokens";
  //JINSU change to


  //JINSU from
  public static void recordTokenSize() {
    TokenMetadata tokenMetadata = ss.getTokenMetadata();
    int tokenSize = tokenMetadata.sortedTokens().size();
    InetAddress node = DatabaseDescriptor.getListenAddress();
    byte[] addr = node.getAddress();
    int nodeName = (int) addr[3];
    ...
    String filepath = tokenPath + "/nodeToken" + nodeName;
    //create the token file until it writes..
    String msg = "TokenSizeTest " + Integer.toString(tokenSize);
    if(Util.stringToFileContent(msg, filepath)) {
      Util.debug("node" + nodeName + " recorded Tokensize " + tokenSize);
    } else {
      Util.debug("node failed to record Tokensize.");
    }
  }
  //JINSU to
  ...
  private class GossipTimerTask extends TimerTask
  {
    public void run()
    {
      ...
      //JINSU change from
      if (nodeFlag.exists()) { 
        Util.debug("- Nodes connected, Gossiper returning ");
        return;
        //Util.sleep(10000);
      } else if(expFlag.exists()) {
        Util.debug("- Experiment running, Gossiper returning ");
        return;
      } else {
        Util.debug("- Nodes not connected...");
      }
            //JINSU change to
    ...
    */
    ...
    boolean sendGossip(Message message, Set<InetAddress> epSet)
    {
        ...
        //jinsu changes from
        Util.sleepWhileExperimentRunning("sendGossip"); 
        //jinsu changes to
    ...
    */
    ...
    void markAlive(InetAddress addr, EndPointState localState)
    {
        ...
        if ( !localState.isAlive() )
        {
            ...
            //JINSU critical. Driver waitForRegistration checks 
            //for this tag to determine whether each node is ready.
            logger_.info(addr + "is cassReady");
        }
    }
